...
POINTSIZE_MARCA_AGUA=$((POINTSIZE - 10))

if [ "$POINTSIZE_MARCA_AGUA" -lt 0 ]; then
  POINTSIZE_MARCA_AGUA=40
fi
...
convert -font "$FONT" -pointsize "$POINTSIZE" -fill "$COLOR" \
    ...
    -pointsize "$POINTSIZE_MARCA_AGUA" \
    -draw "text 10,$Y_MARCA_AGUA 'QSL Generated by PITEA'" \
    "$OFILE" "$OFILE"
