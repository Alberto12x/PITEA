...
\usepackage{verbatim}  % Paquete para verbatim

% Paquete de codificación UTF-8
\usepackage[utf8]{inputenc} % Para que pinte bien las tildes

% Paquete para gráficos, figuras, tablas y más
\usepackage{graphicx}
\usepackage{epsfig}
...
% Paquete para subfiguras (para figuras dentro de figuras)
\usepackage{subfig}

% Paquete para manejar índices
\usepackage{makeidx}

% Paquete para citas y bibliografía
\usepackage[round]{natbib}

% Paquete para referencias de URL
\usepackage{url}

% Paquete para enlaces en PDF
\usepackage{hyperref}

% Paquete para incluir código resaltado (sin listings)
\usepackage{minted}

% Paquete para utilizar colores
\usepackage{xcolor}

% Paquete para trabajar con cuadros de texto personalizados
\usepackage[most]{tcolorbox}

% Paquete para manejar la tipografía y estilo del documento
\usepackage{titlesec}

% Paquete para trabajar con la numeración y páginas
\usepackage{geometry}
\geometry{top=3cm,bottom=3cm,left=3cm,right=3cm}

% Paquete para el idioma y acentos
\usepackage[english,activeacute,spanish]{babel}


% Eliminar listings y mantener únicamente minted
% Si tenías configuraciones de listings, ¡eliminarlas!

% Declaración de títulos de capítulos, secciones, etc.
\usepackage[Lenny]{fncychap}

% Incluir la opción final para generar los documentos PDF
\usepackage[final]{pdfpages}

% Otros paquetes adicionales
\usepackage{lipsum} % Para generar texto ficticio
\usepackage{placeins} % Para controlar la ubicación de las figuras
\usepackage{siunitx}  % Para el manejo de unidades y valores científicos

% Correcciones adicionales para cualquier otro paquete que sea necesario
\sisetup{separate-uncertainty=true}  % Configuración de siunitx

\setminted{
    frame=single,                     % Marco alrededor del código
    bgcolor=gray!10,                  % Fondo gris claro
    ...
    breaklines=true,                  % Activar los saltos de línea automáticos
}
\usepackage{xurl}
